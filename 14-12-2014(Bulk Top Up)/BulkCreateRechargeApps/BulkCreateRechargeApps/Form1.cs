using BulkCreateRechargeApps.com.sslwireless.vrtest;
...
namespace BulkCreateRechargeApps
{
    public partial class CreateRechargeApp1 : Form
    {
            ...
            label1.Text = Msg;     
                ...
                {                   
                    try
                    {
                      ...
                      //  var ProcessingData = db.RechargeInfoTbls.AsEnumerable().Where(g => g.create_recharge_Status == "processing" && g.id % Convert.ToInt32(total_exe) == Convert.ToInt32(appflag)).OrderBy(g => g.id).Take(20).ToList();
                       // var ProcessingData = db.RechargeInfoTbls.AsEnumerable().Where(g => g.create_recharge_Status == "processing") .OrderByDescending(g => g.id).Take(Convert.ToInt32(limit)).ToList();
                        ...
                        if (ProcessingData.Count() != 0)
                            {
                                ...
                                {
                                   // var CheckId =CheckIdFromFile(i.id);
                                  // SaveIdToFile(i.id);
                                  //  SaveFileWithId(i.id);
                                  //  RemoveFileWithId(i.id);
        ...
        private string  CheckMobileNo(RechargeInfoTbl singleRow)
        {
            string reply = "";
            int  s ;
            if (singleRow.operator_id >= 1 && singleRow.operator_id <= 6 && int.TryParse(singleRow.recepient_msisdn,out s) && singleRow.recepient_msisdn.StartsWith("88") ? singleRow.recepient_msisdn.Length == 13 : singleRow.recepient_msisdn.Length == 11)
            {
